...
export const Restock_in = ({ user }) => {
  ...
  const columns = [
    ...
    { key: 'importRequestCode', label: 'Import Request code', width: 'w-36' },
    { key: 'importCommandCode', label: 'Import Command code', width: 'w-36' },
    { key: 'dateCreate', label: 'Date Create', width: 'w-24' },
    { key: 'importWarehouse', label: 'Import warehouse', width: 'w-40' },
    { key: 'contract', label: 'Contract', width: 'w-28' },
    { key: 'creator', label: 'Creator', width: 'w-28' },
    { key: 'unitRequests', label: 'Unit Requests', width: 'w-28' },
    { key: 'unitReceive', label: 'Unit Receive', width: 'w-28' },
    { key: 'dateDelivery', label: 'Date Delivery', width: 'w-24' },
  ...
  const processImport = async (newRawData) => {
    ...
    const currentCodes = new Set(data.map(item => item.importRequestCode));
    ...
    const processedNewData = filteredData.map((item, index) => {
      ...
      return {
        ...
        no: index + 1,
        importRequestCode: item.importRequestCode || '',
        importCommandCode: item.importCommandCode || '',
        dateCreate: item.dateCreate || '',
        importWarehouse: item.importWarehouse || '',
    ...
    if (completedCodesArray.length > 0) {
      const newCompletions = completedCodesArray.map(code => {
        const foundItem = data.find(item => item.importRequestCode === code);
        return { 
  ...
  const calculateKPIData = useMemo(() => {
    ...
    activeRecords.forEach(item => {
      ...
      if (unit && VALID_UNITS.includes(unit)) {
        ...
        unitGroups[unit].codes.add(item.importRequestCode);
  ...
  const deleteSelectedRows = () => {
    ...
    if (window.confirm(`⚠️ Delete ${selectedRows.size} row(s)?`)) {
      const deletedCodes = data.filter(item => selectedRows.has(item.id)).map(item => item.importRequestCode);
      const newCompletions = deletedCodes.map(code => ({
        importRequestCode: code, completedAt: new Date().toISOString(),
        unit: data.find(item => item.importRequestCode === code)?.unit || 'UNKNOWN'
      }));
      setCompletionHistory(prev => [...newCompletions, ...prev]);
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Restock In Data');
    XLSX.writeFile(wb, `restock_in_data_${new Date().toISOString().split('T')[0]}.xlsx`);
  ...
  const exportKPItoExcel = () => {
    ...
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Restock In KPI');
    XLSX.writeFile(wb, `restock_in_kpi_${new Date().toISOString().split('T')[0]}.xlsx`);
  ...
  useEffect(() => {
    if (alarmItems.length > 0) {
      ...
      try {
        const stored = sessionStorage.getItem('shown_restock_in_alarms');
      ...
      if (newAlarms.length > 0) {
        ...
        try {
          sessionStorage.setItem('shown_restock_in_alarms', JSON.stringify([...shownIds]));
        } catch (e) {}
      }
    }
  }, [alarmItems]);
        ...
        <div className="bg-white rounded-2xl shadow-2xl max-w-5xl w-full mx-4 border border-gray-100 animate-scaleIn">
          ...
          <div className="p-6 bg-white">
            <textarea 
              ...
              placeholder="Paste your system data here...&#10;&#10;Format: Import Request code, Import Command code, Date Create, Import warehouse, Contract, Creator, Unit Requests, Unit Receive, Date Delivery, Status CA&#10;&#10;Note: Only YCNKGIS_ format with valid units will be imported.&#10;&#10;Example:&#10;YCNKGIS_STU_SOS03/26/000086		24/06/2026	GIS_STU_STOCK_XL_SOSTEAM03		stu_tec_sokimsan	GIS_STU_SOS_TEAM03	GIS_STU_SOS_TEAM03	24/06/2026	Unsigned" 
                  ...
                  <div className="flex justify-between items-start gap-4">
                    <div className="text-xs">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="text-gray-500 font-mono">| Request: {item.importRequestCode}</span>
                      </div>
                      <div className="text-gray-600 mt-1">📅 Date: {item.dateCreate} | Year: {item.year} | ⏰ Delay: <span className="font-bold text-rose-600 font-mono">+{item.daysDiff} days</span></div>
                      <div className="text-[11px] text-gray-500 mt-1">Warehouse: {item.importWarehouse} | Status CA: {item.statusCA}</div>
                    </div>
      ...
      <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-blue-600 to-blue-800 px-6 py-5">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              ...
              <p className="text-blue-100 mt-1 text-sm">**Recall Process → ដំណើរការដកសម្ភារៈត្រឡប់ចូលស្តុក METFONE**</p>
            </div>
            <div className="flex gap-2">
              {!isUnitUser && (
                ...
                return (
                  <tr key={item.id} className={`${isAlarm ? 'bg-rose-50' : ''} ${selectedRows.has(item.id) ? 'bg-blue-50' : ''} hover:bg-gray-50 transition-colors`}>
                    ...
                    <td className="px-2 py-1.5 text-xs font-mono break-all">
                      {editingCell?.id === item.id && editingCell?.field === 'importRequestCode' ? (
                        <input type="text" defaultValue={item.importRequestCode} autoFocus onBlur={(e) => saveEdit(item.id, 'importRequestCode', e.target.value)} onKeyDown={(e) => handleKeyPress(e, item.id, 'importRequestCode')} className="w-full px-1.5 py-0.5 border rounded-xl text-xs bg-white" />
                      ) : (
                        <div onClick={() => startEdit(item.id, 'importRequestCode', item.importRequestCode)} className="cursor-pointer hover:bg-gray-100 px-1 py-0.5 rounded transition-colors">{item.importRequestCode || '-'}</div>
                      )}
                    </td>
                    <td className="px-2 py-1.5 text-xs font-mono break-all">
                      {editingCell?.id === item.id && editingCell?.field === 'importCommandCode' ? (
                        <input type="text" defaultValue={item.importCommandCode} autoFocus onBlur={(e) => saveEdit(item.id, 'importCommandCode', e.target.value)} onKeyDown={(e) => handleKeyPress(e, item.id, 'importCommandCode')} className="w-full px-1.5 py-0.5 border rounded-xl text-xs bg-white" />
                      ) : (
                        <div onClick={() => startEdit(item.id, 'importCommandCode', item.importCommandCode)} className="cursor-pointer hover:bg-gray-100 px-1 py-0.5 rounded transition-colors">{item.importCommandCode || '-'}</div>
                      )}
                    </td>
                    ...
                    <td className="px-2 py-1.5 text-xs break-words">
                      {editingCell?.id === item.id && editingCell?.field === 'importWarehouse' ? (
                        <input type="text" defaultValue={item.importWarehouse} autoFocus onBlur={(e) => saveEdit(item.id, 'importWarehouse', e.target.value)} onKeyDown={(e) => handleKeyPress(e, item.id, 'importWarehouse')} className="w-full px-1.5 py-0.5 border rounded-xl text-xs bg-white" />
                      ) : (
                        <div onClick={() => startEdit(item.id, 'importWarehouse', item.importWarehouse)} className="cursor-pointer hover:bg-gray-100 px-1 py-0.5 rounded transition-colors">{getWarehouseBadge(item.importWarehouse)}</div>
                      )}
                    </td>
                    <td className="px-2 py-1.5 text-xs break-words">
                      {editingCell?.id === item.id && editingCell?.field === 'contract' ? (
                        <input type="text" defaultValue={item.contract} autoFocus onBlur={(e) => saveEdit(item.id, 'contract', e.target.value)} onKeyDown={(e) => handleKeyPress(e, item.id, 'contract')} className="w-full px-1.5 py-0.5 border rounded-xl text-xs bg-white" />
                      ) : (
                        <div onClick={() => startEdit(item.id, 'contract', item.contract)} className="cursor-pointer hover:bg-gray-100 px-1 py-0.5 rounded transition-colors">{item.contract || '-'}</div>
                      )}
                    </td>
                    <td className="px-2 py-1.5 text-xs break-words">
                    ...
                    <td className="px-2 py-1.5 text-xs break-words">
                      {editingCell?.id === item.id && editingCell?.field === 'unitRequests' ? (
                        <input type="text" defaultValue={item.unitRequests} autoFocus onBlur={(e) => saveEdit(item.id, 'unitRequests', e.target.value)} onKeyDown={(e) => handleKeyPress(e, item.id, 'unitRequests')} className="w-full px-1.5 py-0.5 border rounded-xl text-xs bg-white" />
                      ) : (
                        <div onClick={() => startEdit(item.id, 'unitRequests', item.unitRequests)} className="cursor-pointer hover:bg-gray-100 px-1 py-0.5 rounded transition-colors">{item.unitRequests || '-'}</div>
                      )}
                    </td>
                    <td className="px-2 py-1.5 text-xs break-words">
                      {editingCell?.id === item.id && editingCell?.field === 'unitReceive' ? (
                        <input type="text" defaultValue={item.unitReceive} autoFocus onBlur={(e) => saveEdit(item.id, 'unitReceive', e.target.value)} onKeyDown={(e) => handleKeyPress(e, item.id, 'unitReceive')} className="w-full px-1.5 py-0.5 border rounded-xl text-xs bg-white" />
                      ) : (
                        <div onClick={() => startEdit(item.id, 'unitReceive', item.unitReceive)} className="cursor-pointer hover:bg-gray-100 px-1 py-0.5 rounded transition-colors">{item.unitReceive || '-'}</div>
                      )}
                    </td>
                    <td className="px-2 py-1.5 text-xs font-mono text-center">
                      {editingCell?.id === item.id && editingCell?.field === 'dateDelivery' ? (
                        <input type="text" defaultValue={item.dateDelivery} autoFocus onBlur={(e) => saveEdit(item.id, 'dateDelivery', e.target.value)} onKeyDown={(e) => handleKeyPress(e, item.id, 'dateDelivery')} className="w-full px-1.5 py-0.5 border rounded-xl text-xs font-mono bg-white" />
                      ) : (
                        <div onClick={() => startEdit(item.id, 'dateDelivery', item.dateDelivery)} className="cursor-pointer hover:bg-gray-100 px-1 py-0.5 rounded font-mono">{item.dateDelivery || '-'}</div>
                      )}
                    </td>
                ...
                <tr>
                  <td colSpan={columns.length + 1} className="px-6 py-12 text-center text-gray-500">
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <p className="text-sm text-gray-400">Filters: YCNKGIS_ + Unsigned + Valid Units</p>
                    </div>
                  </td>
                </tr>
              )}
        ...
        <div className="bg-gray-50 px-6 py-3 border-t text-sm text-gray-500 flex justify-between flex-wrap gap-2">
          ...
          {/* <div className="flex gap-3 flex-wrap">
            <span className="flex items-center gap-1"><span className="w-3 h-3 rounded-full bg-emerald-500"></span>Valid Units</span>
